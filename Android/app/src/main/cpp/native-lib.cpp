...
JNI_API(jboolean, startDemo, jstring path_to_jni_file)
{
	...
	static onceToken s_token([] { s_th.detach(); });
	return true;
};
...
JNI_API(jlong, createMediaPlayer, jstring url, jobject callback)
{
	...
	player->setOnPlayResult([weakPlayer, globalWeakRef](const SockException& ex) {
		...
		if (viedoTrack) {
			viedoTrack->addDelegate(std::make_shared<FrameWriterInterfaceHelper>([globalWeakRef](const Frame::Ptr& frame) { emitEvent((jobject)globalWeakRef, "onData", "(L" MediaFrameSign ";)V", makeJavaFrame(env, frame)); }));
		}

		auto audioTrack = strongPlayer->getTrack(TrackAudio);
		if (audioTrack) {
			audioTrack->addDelegate(std::make_shared<FrameWriterInterfaceHelper>([globalWeakRef](const Frame::Ptr& frame) { emitEvent((jobject)globalWeakRef, "onData", "(L" MediaFrameSign ";)V", makeJavaFrame(env, frame)); }));
		}
	});
