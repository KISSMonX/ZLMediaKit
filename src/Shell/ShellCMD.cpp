...
namespace mediakit {
	...
	class CMD_media : public CMD {
		...
		CMD_media()
		{
			_parser.reset(new OptionParser([](const std::shared_ptr<ostream>& stream, mINI& ini) {
				MediaSource::for_each_media([&](const string& schema, const string& vhost, const string& app, const string& streamid, const MediaSource::Ptr& media) {
					...
					if (ini.find("list") != ini.end()) {
						//列出源
						(*stream) << "\t" << schema << "/" << vhost << "/" << app << "/" << streamid << "\r\n";
						return;
					}
					...
					if (ini.find("kick") != ini.end()) {
						...
						do {
							...
							(*stream) << "\t踢出成功:" << schema << "/" << vhost << "/" << app << "/" << streamid << "\r\n";
							return;
						} while (0);
						(*stream) << "\t踢出失败:" << schema << "/" << vhost << "/" << app << "/" << streamid << "\r\n";
						return;
					}
				});
			}));
	...
	static onceToken s_token([]() { REGIST_CMD(media); }, nullptr);
