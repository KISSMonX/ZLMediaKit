...
static const uint8_t sg_aau8DefaultScaling4[2][16] = {{6, 13, 20, 28, 13, 20, 28, 32, 20, 28, 32, 37, 28, 32, 37, 42}, {10, 14, 20, 24, 14, 20, 24, 27, 20, 24, 27, 30, 24, 27, 30, 34}};

static const uint8_t sg_aau8DefaultScaling8[2][64] = {{6, 10, 13, 16, 18, 23, 25, 27, 10, 11, 16, 18, 23, 25, 27, 29, 13, 16, 18, 23, 25, 27, 29, 31, 16, 18, 23, 25, 27, 29, 31, 33, 18, 23, 25, 27, 29, 31, 33, 36, 23, 25, 27, 29, 31, 33, 36, 38, 25, 27, 29, 31, 33, 36, 38, 40, 27, 29, 31, 33, 36, 38, 40, 42}, {9, 13, 15, 17, 19, 21, 22, 24, 13, 13, 17, 19, 21, 22, 24, 25, 15, 17, 19, 21, 22, 24, 25, 27, 17, 19, 21, 22, 24, 25, 27, 28, 19, 21, 22, 24, 25, 27, 28, 30, 21, 22, 24, 25, 27, 28, 30, 32, 22, 24, 25, 27, 28, 30, 32, 33, 24, 25, 27, 28, 30, 32, 33, 35}};
...
const uint8_t g_au8FfZigzagDirect[64] = {0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40, 48, 41, 34, 27, 20, 13, 6, 7, 14, 21, 28, 35, 42, 49, 56, 57, 50, 43, 36, 29, 22, 15, 23, 30, 37, 44, 51, 58, 59, 52, 45, 38, 31, 39, 46, 53, 60, 61, 54, 47, 55, 62, 63};
...
static int getBits(void* pvHandle, int iN)
{
	...
	if (iN == MAX_LEN) {
		RPT(RPT_DBG, "12(ptPtr->iBitPos(:%d) + iN(:%d)) > ptPtr->iTotalBit(:%d)!!! ", ptPtr->iBitPos, iN, ptPtr->iTotalBit);
	...
	iRet = iRet << 8;
	iRet = ((uint32_t)au8Temp[1] << 24) | ((uint32_t)au8Temp[2] << 16) | ((uint32_t)au8Temp[3] << 8) | au8Temp[4];
...
static void decodeScalingList(void* pvBuf, uint8_t* pu8Factors, int iSize, const uint8_t* pu8JvtList, const uint8_t* pu8FallbackList)
{
	int	    i;
	int	    iLast   = 8;
	int	    iNext   = 8;
...
int decodeScalingMatrices(void* pvBuf, T_SPS* ptSps, T_PPS* ptPps, int iIsSps, uint8_t (*pau8ScalingMatrix4)[16], uint8_t (*pau8ScalingMatrix8)[64])
{
	int	    iFallbackSps    = !iIsSps && ptSps->iScalingMatrixPresent;
	const uint8_t* pau8Fallback[4] = {iFallbackSps ? ptSps->aau8ScalingMatrix4[0] : sg_aau8DefaultScaling4[0], iFallbackSps ? ptSps->aau8ScalingMatrix4[3] : sg_aau8DefaultScaling4[1], iFallbackSps ? ptSps->aau8ScalingMatrix8[0] : sg_aau8DefaultScaling8[0], iFallbackSps ? ptSps->aau8ScalingMatrix8[3] : sg_aau8DefaultScaling8[1]};
	if (getOneBit(pvBuf)) {
...
static inline int decodeVuiParameters(void* pvBuf, T_SPS* ptSps)
{
	...
	if (iAspectRatioInfoPresentFlag) {
		...
		ptSps->tSar.num = ptSps->tSar.den = 0;
	}
	...
	if (ptSps->iVclHrdParametersPresentFlag)
		if (decodeHrdPAarameters(pvBuf, ptSps) < 0)
			return -1;
	if (ptSps->iNalHrdParametersPresentFlag || ptSps->iVclHrdParametersPresentFlag)
...
int h264DecSeqParameterSet(void* pvBufSrc, T_SPS* ptSps)
{
	...
	if (ptSps->iProfileIdc == 100 || // High profile
		...
		if (ptSps->iBitDepthLuma < 8 || ptSps->iBitDepthLuma > 14 || ptSps->iBitDepthChroma < 8 || ptSps->iBitDepthChroma > 14) {
			RPT(RPT_ERR, "illegal bit depth value (%d, %d)\n", ptSps->iBitDepthLuma, ptSps->iBitDepthChroma);
			iRet = -1;
			goto exit;
		}
		ptSps->iTransformBypass = getOneBit(pvBuf);
		decodeScalingMatrices(pvBuf, ptSps, NULL, 1, ptSps->aau8ScalingMatrix4, ptSps->aau8ScalingMatrix8);
	}
	else {
	...
	if (iLog2MaxFrameNumMinus4 < MIN_LOG2_MAX_FRAME_NUM - 4 || iLog2MaxFrameNumMinus4 > MAX_LOG2_MAX_FRAME_NUM - 4) {
	...
	if (ptSps->iRefFrameCount > H264_MAX_PICTURE_COUNT - 2 || ptSps->iRefFrameCount > 16U) {
	...
	if (ptSps->iCrop) {
		...
		if (1) {
			...
			int hsub   = (ptSps->iChromaFormatIdc == 1 || ptSps->iChromaFormatIdc == 2) ? 1 : 0;
			int step_x = 1 << hsub;
			...
			if (uiCropLeft > (unsigned)INT_MAX / 4 / step_x || uiCropRight > (unsigned)INT_MAX / 4 / step_x || uiCropTop > (unsigned)INT_MAX / 4 / step_y || uiCropBottom > (unsigned)INT_MAX / 4 / step_y || (uiCropLeft + uiCropRight) * step_x >= width || (uiCropTop + uiCropBottom) * step_y >= height) {
		...
		ptSps->uiCropLeft = ptSps->uiCropRight = ptSps->uiCropTop = ptSps->uiCropBottom = ptSps->iCrop = 0;
	}
