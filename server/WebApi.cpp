...
#define API_ARGS HttpSession::KeyValue &headerIn, HttpSession::KeyValue &headerOut, ApiArgsType &allArgs, Json::Value &val
...
#define API_REGIST_INVOKER(field, name, ...) s_map_api.emplace("/index/" #field "/" #name, [](API_ARGS, const HttpSession::HttpResponseInvoker& invoker) __VA_ARGS__);
...
namespace API {
	typedef enum { InvalidArgsFailed = -300, SqlFailed = -200, AuthFailed = -100, OtherFailed = -1, Success = 0 } ApiErr;
...
class InvalidArgs : public ApiRetException {
    public:
	InvalidArgs(const char* str)
		: ApiRetException(str, API::InvalidArgsFailed)
	{
	}
	~InvalidArgs() = default;
};
...
static inline void addHttpListener()
{
	...
	NoticeCenter::Instance().addListener(nullptr, Broadcast::kBroadcastHttpRequest, [](BroadcastHttpRequestArgs) {
		...
		if (api_debug) {
			auto newInvoker = [invoker, parser, allArgs](const string& codeOut, const HttpSession::KeyValue& headerOut, const string& contentOut) {
				stringstream ss;
				for (auto& pr : allArgs) {
...
template<typename Args, typename First> bool checkArgs(Args&& args, First&& first)
{
	return !args[first].empty();
}

template<typename Args, typename First, typename... KeyTypes> bool checkArgs(Args&& args, First&& first, KeyTypes&&... keys)
{
...
void installWebApi()
{
	...
	API_REGIST(api, getMediaList, {
		...
		val["code"] = 0;
		val["msg"]  = "success";
		MediaSource::for_each_media([&](const string& schema, const string& vhost, const string& app, const string& stream, const MediaSource::Ptr& media) {
	...
	API_REGIST_INVOKER(api, addStreamProxy, {
		...
		//添加拉流代理
		PlayerProxy::Ptr player(new PlayerProxy(allArgs["vhost"], allArgs["app"], allArgs["stream"], allArgs["enable_hls"], allArgs["enable_mp4"]));
